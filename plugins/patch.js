  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`QUEEN MAKIMA👑- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing QUEEN MAKIMA 👑
│└────────────┈ ⳹        
│*©2024-2099 ES TEAMS*
└─────────────────┈ ⳹*
${readmore}`;
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*QUEEN MAKIMA 👑 ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `𝕼𝖚𝖊𝖊𝖓 𝕸𝖆𝖐𝖎𝖒𝖆 👑 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\n𝕼𝖚𝖊𝖊𝖓 𝕸𝖆𝖐𝖎𝖒𝖆 👑`;
        ...
        async () => {
          ...
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `QUEEN MAKIMA 👑 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds\n\n*Fact:*\n${fact.result.fact}\n\n+QUEEN MAKIMA 👑`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*QUEEN MAKIMA 👑 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "QUEEN MAKIMA 👑 Running Since",
        body: message,
        footerText: "QUEEN MAKIMA 👑 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("QUEEN_MAKIMA_FANCY_TEXT")
        ) {
