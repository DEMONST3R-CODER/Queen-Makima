...
smd(
  ...
  async (message) => {
    try {
      let menuMessage = ` 
➮QUEEN MAKIMA RUNTIME - ${runtime(process.uptime())} 
➮QUEEN MAKIMA DATE - ${message.date} 
➮QUEEN MAKIMA NOW TIME - ${message.time} 
➮FOUNDER- *꧁𝕿𝖍𝖚𝖌𝖓𝖎𝖋𝖎𝖈𝖊𝖓𝖙ᵈᵒᵒᵐ 𒆜𓊉꧂ ☠️*
➮OWNER- ${Config.ownername} 
➮NUMBER - ${owner.split(",")[0]} 
➮MEMO - ${formatp(os.totalmem() - os.freemem())} 
      \n *QUEEN MAKIMA SIMPLE WHATSAPP BOT*\n\n ${readmore} 
╭──❰ *QUEEN MAKINA ALL MENU* ❱ 
│🎗 Lɪꜱᴛ 
│🎗 Cᴀᴛᴇɢᴏʀʏ 
│🎗 Hᴇʟᴘ 
│🎗 Aʟɪᴠᴇ 
...
astro_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*Queen_Makima Ping!!!*");
...
astro_patch.cmd(
  ...
  async (message) => {
    try {
      ...
      let listMessage = `\n  
╭━━〘 * ${Config.botname} * 〙    
┃ 🎗 QUEEN MAKIMA PREFIX: ${Config.HANDLERS}
┃ 🎗 QUEEN MAKIMA OWNER: ${Config.ownername}
┃ 🎗 QUEEN MAKIMA COMMAND: ${commands.length}
┃ 🎗 QUEEN MAKIMA UPTIME: ${runtime(process.uptime())}
