...
smd({
    ...
    async({reply, chat ,isGroup, sender,error}) => {
      try{
        ...
        const daily  = await eco.daily(sender, "QUEEN_MAKIMA", 500); //give 500 for daily, can be changed
        if (daily.cd) {
...
async(message,match) => {
  try{
   ...
   const balance  = await eco.balance(user, "QUEEN_MAKIMA")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         ...
         const add1 = eco.giveCapacity(user,"QUEEN_MAKIMA 👑", 1000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "QUEEN_MAKIMA", 10000);
          const add3 = eco.giveCapacity(user, "QUEEN_MAKIMA", 10000000);
